...
│   │   ├── baseof.html   # Base template with <head>, Tailwind config, global styles
...
├── static/           # Static assets (images, files) - currently empty
...
1. **baseof.html** - Base template loaded for all pages
   ...
   - Configures custom theme colors (primary: indigo-600, accent: emerald-500)
   - Loads Inter font from Google Fonts
   - Defines global styles and CSS variables
...
Key config sections:
- `baseURL` - Site URL (currently example.org)
...
3. **Styling changes**: 
   ...
   - Tailwind config is in the `<script>` tag in baseof.html
